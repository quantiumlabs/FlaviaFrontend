...
import { LogOut, Settings, MessageSquare, Check, X, ChevronDown, ChevronUp } from 'lucide-react';
...
const MapProfileSection = ({ user, onLogout, isOpen }) => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      console.log('Modification requests:', data); // Debug log
      setRequests(data);
    } catch (err) {
  ...
  const parseMediaUrls = (mediaUrlsString) => {
    try {
      return mediaUrlsString ? JSON.parse(mediaUrlsString) : [];
    } catch (err) {
