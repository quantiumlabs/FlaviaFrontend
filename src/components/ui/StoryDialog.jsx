        ...
        <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto bg-amber-50 border-4 border-orange-500 sm:rounded-none rounded-none shadow-[8px_8px_0_0_rgba(249,115,22,0.5)] p-8">
          <DialogHeader>
            <div className="flex flex-wrap items-center gap-4 mb-4">
              <span
                className={`inline-flex items-center gap-2 px-3 py-2 text-[8px] font-['Press_Start_2P'] border-2 ${badge.class}`}
              >
                <span className="text-sm">{badge.icon}</span>
                {badge.text}
              </span>
              <span className="text-[8px] font-['Press_Start_2P'] text-slate-500">
                {getTimeAgo(new Date(story.createdAt))}
              </span>
            </div>
            <DialogTitle>
              <div className="flex flex-wrap gap-3 mt-2">
                {story.type === "COLLABORATIVE" ? (
                  [
                    ...
                    <div
                      key={username}
                      className="flex items-center gap-2 bg-white border-2 border-slate-300 px-3 py-2 shadow-sm"
                    >
                      ...
                      <span className="text-[10px] font-['Press_Start_2P'] text-slate-800 uppercase">
                        {username}
                      </span>
                    </div>
                  ))
            ...
            <div className="mt-4 bg-red-100 border-4 border-red-500 p-4">
              <div className="flex items-start">
                ...
                <p className="text-[8px] font-['Press_Start_2P'] text-red-800 leading-relaxed uppercase">
                  ATENCAO: Esta historia contem links ou codigos HTML suspeitos.
                  Pode ser uma tentativa de phishing. Tenha cuidado.
                </p>
              </div>
            </div>
          )}
          ...
          {story.mediaUrls?.length > 0 && (
            <div className="mt-6 grid gap-4 grid-cols-1 sm:grid-cols-2">
              {story.mediaUrls.map((url, index) => (
                <div
                  key={index}
                  className="relative border-4 border-slate-300 bg-white p-1"
                >
